...
class Character extends FunkinSprite implements IBeatReceiver implements IOffsetCompatible implements IPrePostDraw {
	...
	public override function playAnim(AnimName:String, ?Force:Bool, Context:PlayAnimContext = NONE, Reversed:Bool = false, Frame:Int = 0)
	{
		...
		if (event.context == SING && getAnimName() == event.animName && isAnimFinished()) {
			event.force = false;
		}
	...
	public function buildCharacter(xml:Access) {
		...
		if (xml.x.exists("filterQuality")) {
			...
			if (qualityInt != null) animateSettings.filterQuality = cast(qualityInt, FilterQuality);
			else {
				var values = ["high", "medium", "low", "rudy"];
				var index = values.indexOf(val.toLowerCase());
				if (index > -1) animateSettings.filterQuality = cast(index, FilterQuality);
			}
		}
	...
	public static var characterProperties:Array<String> = ["x", "y", "sprite", "scale", "antialiasing", "flipX", "camx", "camy", "isPlayer", "icon", "color", "gameOverChar", "holdTime", "applyStageMatrix"];
	public static var characterAnimProperties:Array<String> = ["name", "anim", "label", "x", "y", "fps", "loop", "indices"];
	...
	public inline function buildXML(?animsOrder:Array<String>):Xml {
		...
		if (!antialiasing) xml.set("antialiasing", "false");
		if (isPlayer) xml.set("isPlayer", "true");
		...
		} else anims = Lambda.array(animDatas);
		...
		for (anim in anims) {
			...
			if (anim.loop) animXml.set("loop", "true");
			...
			offset.putWeak();
			if (anim.indices.length > 0) animXml.set("indices", CoolUtil.formatNumberRange(anim.indices));
			if (anim.label) animXml.set("label", "true");
	...
	public inline function getIcon() return (icon != null) ? icon : curCharacter;
	public function getAnimOrder() return [for(a in xml.nodes.anim) if(a.has.name) a.att.name];
	...
	@:noCompletion private function set_danceInterval(v:Int) return beatInterval = v;
	@:noCompletion private function get_danceInterval() return beatInterval;
	...
	private function set_script(script:Script):Script {
		...
		if(lastIndex >= 0) {
			if(script == null) scripts.scripts.splice(lastIndex, 1);
			else scripts.scripts[lastIndex] = script;
		} else if(script != null) scripts.insert(0, script);
	...
	public static function getXMLFromCharName(character:OneOfTwo<String, Character>):Access {
		...
		while (true) {
			...
			if (!Assets.exists(xmlPath)) {
				...
				if (char != null) char.curCharacter = character;
				continue;
			}
