...
class StrumLine extends FlxTypedGroup<Strum> {
	...
	private inline function get_ghostTapping() {
		if (this.ghostTapping != null) return this.ghostTapping;
		if (PlayState.instance != null) return PlayState.instance.ghostTapping;
		return false;
	}

	private inline function set_ghostTapping(b:Bool):Bool return this.ghostTapping = b;
	...
	public function generate(strumLine:ChartStrumLine, ?startTime:Float) {
		...
		if (strumLine.notes != null) for(note in strumLine.notes) {
			if (startTime != null && startTime > note.time) continue;
			total++;
		...
		if (strumLine.notes != null) for(note in strumLine.notes) {
			...
			if (note.sLen > Conductor.stepCrochet * 0.75) {
				...
				while(len > 10) {
					...
					len -= curLen;
					if (prev != null && prev.sustainParent != null) prev.sustainParent.tailCount++;
				}
			}
		}
		notes.sortNotes();
	...
	public function updateNote(daNote:Note) {
		...
		if (__updateNote_event.__updateHitWindow) {
			...
			daNote.canBeHit = (daNote.strumTime > __updateNote_songPos - (hitWindow * daNote.latePressWindow) && daNote.strumTime < __updateNote_songPos + (hitWindow * daNote.earlyPressWindow));
			if (daNote.strumTime < __updateNote_songPos - hitWindow && !daNote.wasGoodHit) daNote.tooLate = true;
		}
		...
		if (daNote.isSustainNote) daNote.updateSustain(__updateNote_event.strum);
	}
	...
	inline function __inputProcessPressed(note:Note) {
	...
	inline function __inputProcessJustPressed(note:Note) {
	...
	public function updateInput(id:Int = 0) {
		...
		if (__notePerStrum.length != membersLength)
			__notePerStrum = cast new haxe.ds.Vector(membersLength);
		...
		if (__pressed.contains(true)) {
			...
			for (c in characters)
				if (c.lastAnimContext != DANCE) c.__lockAnimThisFrame = true;
	...
	public inline function generateStrums(amount:Int = -1) {
		...
		for (i in 0...amount) add(createStrum(i));
	}
	...
	public function createStrum(i:Int, ?animPrefix:String, ?spritePath:String, ?playIntroAnimation:Bool) {
		if (animPrefix == null) animPrefix = strumAnimPrefix[i % strumAnimPrefix.length];
		...
		if (data.scrollSpeed != null) babyArrow.scrollSpeed = data.scrollSpeed;
