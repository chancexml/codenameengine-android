...
class Charter extends UIState {
			...
			group.forEach(function(s) {
				...
				if (gridActionType == NONE) {
					if (s is CharterNote) {
						...
						if ((n.hovered || n.sustainDraggable) && mobile.controls.Call.virtualMouse.justReleased) select(cast s);
					} else if (mobile.controls.Call.virtualMouse.justReleased && s.hovered) select(cast s);
				}
			});
		}
	...
	public function updateNoteLogic(elapsed:Float) {
		 ...
		 * NOTE DRAG HANDLING
		 */
		mobile.controls.Call.virtualMouse.getWorldPosition(charterCamera, mousePos);
		if (!gridBackdropDummy.hoveredByChild && !mobile.controls.Call.virtualMouse.pressed)
			gridActionType = NONE;
		selectionBox.visible = false;
		switch(gridActionType) {
			case BOX_SELECTION:
				if (gridBackdropDummy.hoveredByChild) {
					...
					if (isSelecting) {
						...
						if (mobile.controls.Call.virtualMouse.justReleased) isSelecting = false;
					} else {
			...
			case INVALID_DRAG:
				// do nothing, locked
				if (!mobile.controls.Call.virtualMouse.pressed)
					gridActionType = NONE;
			case NOTE_DRAG:
				selectionDragging = mobile.controls.Call.virtualMouse.pressed;
			...
			case NONE:
				if (mobile.controls.Call.virtualMouse.justPressed) 
					mobile.controls.Call.virtualMouse.getWorldPosition(charterCamera, dragStartPos);
				else if (mobile.controls.Call.virtualMouse.justPressedRight) {
				...
				if (gridBackdropDummy.hovered) {
					// AUTO DETECT
					if (mobile.controls.Call.virtualMouse.justPressed) isSelecting = true;
					...
					if (mobile.controls.Call.virtualMouse.justReleased) {
			...
			case DELETE_SELECTION:
				...
				if (mobile.controls.Call.virtualMouse.justReleasedRight) {
					if (deletedNotes.length > 0) {
						undos.addToUndo(CDeleteSelection(deletedNotes.copy()));
					}
					else if (noteDeleteAnims.garbageIcon.alpha <= .5) {
						var mousePos = mobile.controls.Call.virtualMouse.getScreenPosition(uiCamera);
						closeCurrentContextMenu();
	...
	public override function update(elapsed:Float) {
		...
		if (true) {
			...
			if (!topMenuSpr.anyMenuOpened) {
				if (mobile.controls.Call.virtualMouse.wheel != 0 && shouldScroll) {
					if (FlxG.keys.pressed.CONTROL) {
						zoom += 0.25 * mobile.controls.Call.virtualMouse.wheel;
						__camZoom = Math.pow(2, zoom);
					} else if (FlxG.keys.pressed.SHIFT) {
						sideScroll -= 40 * mobile.controls.Call.virtualMouse.wheel;
					} else {
						if (!FlxG.sound.music.playing) {
							Conductor.songPosition -= (__crochet * mobile.controls.Call.virtualMouse.wheel) - Conductor.songOffset;
						}
					}
				}
			}
